...
/**
 * Configures the AIC3204 audio codec for audio input and output.
 */
void setup_aic3204()
{
...
/**
 * Reads in a single sample from the DSP input.
 * First call will always read left channel first,
 * second call will read right.
 */
Int16 read_sample()
{
    Int16 sample;
    EZDSP5502_MCBSP_read(&sample);
    return sample;
}

/**
 * Writes out a single sample to the DSP output.
 * First call will always write left channel first,
 * second call will write right.
 *
 * Amplitude scales the sample down by a multiple
 * of 2 by right shifting. A scale of '0' will not
 * attenuate the signal at all. A scale of 16 will
 * fully attenuate the signal.
 */
void play_sample(Int16 sample, Uint8 amplitude)
{
    EZDSP5502_MCBSP_write(sample >> amplitude);
}

/**
 * Cleans up the codec resetting its parameters.
 */
